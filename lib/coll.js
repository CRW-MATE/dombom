...
const pause_menu = () => {
  let objSup = {
    ...
    Mutiplayer: {
      ...
      [`remove 1 ${interZept ? `out of the ${interZept + 1}` : "unavailable"}`]:
        () => {
          rmvP();
        },
      ...
      [`${interZept + 1} ${interZept < 1 ? "player only" : "players"}`]: 0,
    },
    help: {
      ["press X to jump"]: 0,
...
dialog.draw = () => {
  ...
  drawMultilineText(c, dialog.text, 160, 150, 28);
};
...
const movemento = (obj) => {
  ...
  if (obj.state == "crouching") {
    obj.look == "R"
      ? characters[interZept].CTR.do()
      : characters[interZept].CTL.do();
    obj.velX = 0;
    obj.velY = 0;
    return;
    ...
    if (
      ...
      obj.look == "R"
        ? characters[interZept].CWR.do()
        : characters[interZept].CWL.do();
    } else {
      obj.look == "R"
        ? characters[interZept].CNR.do()
        : characters[interZept].CNL.do();
    }
    return;
    ...
    obj.velX *= 0.9;
    obj.look == "R"
      ? characters[interZept].GTJR.do()
      : characters[interZept].GTJL.do();
  } else if (obj.state == "jumping" && obj.G) {
    obj.look == "R"
      ? characters[interZept].JR.do()
      : characters[interZept].JL.do();
    if (obj.dire == "R") {
      obj.velX = 3;
    } else if (obj.dire == "L") {
      obj.velX = -3;
    ...
    if (
      ...
      if (PlayerBase[interZept].look == "R") {
        characters[interZept].NR.do();
      } else if (PlayerBase[interZept].look == "L") {
        characters[interZept].NL.do();
      } else if (PlayerBase[interZept].look == "D") {
        characters[interZept].ND.do();
      } else if (PlayerBase[interZept].look == "U") {
        characters[interZept].NU.do();
      }
      if (!PlayerBase[interZept].G) {
        obj.velY = 0;
      }
...
class scene {
  constructor(bet, det, vure, deo) {
        ...
        if (
          ...
          switch (B.type) {
            ...
            case "door":
              ...
              if (B.ido) {
                if (PlayerBase[interZept].dire == "U") {
                  B.chkCol(world.dess);
                }
              } else {
                B.chkCol(det);
    ...
    this.collis = function () {
      for (let inz = 0; inz < PlayerBase.length; inz++) {
        PlayerBase[inz].G = vure;
        c.clearRect(0, 0, mx, mh);
        ///collisionn!!
        if (PlayerBase[inz].timeRatekoff) {
          PlayerBase[inz].xi = PlayerBase[inz].x;
          PlayerBase[inz].yi = PlayerBase[inz].y;
          if (PlayerBase[inz].G) {
            PlayerBase[inz].velY -= PlayerBase[inz].G;
          }
          PlayerBase[inz].y -= PlayerBase[inz].velY;
          PlayerBase[inz].x += PlayerBase[inz].velX;

          if (deo == "ycz" && PlayerBase[inz].yc > 0) {
            //mism vragity
            PlayerBase[inz].yc--;
          }
          // the cinematic view "ie camera following you around in the x axis"
          if (PlayerBase[inz].x > mx / 2 || PlayerBase[inz].xc > 0) {
            if (PlayerBase[inz].xc + PlayerBase[inz].x < det + mx / 2) {
              PlayerBase[inz].xc += PlayerBase[inz].velX;
              if (PlayerBase[inz].xc < 0) {
                PlayerBase[inz].xc = 0;
              }
            }
          }
          // the cinematic view "ie camera following you around in the y axis"
          if (PlayerBase[inz].y > mh / 2 || PlayerBase[inz].yc < 0) {
            if (PlayerBase[inz].yc + PlayerBase[inz].y < deo + mh / 2) {
              PlayerBase[inz].yc -= PlayerBase[inz].velY;
              if (PlayerBase[inz].yc < 0) {
                PlayerBase[inz].yc = 0;
              }
            }
          }
        ...
        if (PlayerBase[inz].awake) {
          if (PlayerBase[inz].timeRatekoff) {
            PlayerBase[inz].clouds += 10 * Math.abs(PlayerBase[inz].timeRate);
            PlayerBase[inz].time += PlayerBase[inz].timeRate;
          }
          if (PlayerBase[inz].clouds > mx) {
            PlayerBase[inz].clouds = 0;
          }
          if (PlayerBase[inz].time > 255 || PlayerBase[inz].time < 0) {
            PlayerBase[inz].timeRate = -PlayerBase[inz].timeRate;
          }
          c.fillStyle = `rgba(
          ${76 - Math.round(PlayerBase[inz].time)},
          ${186 - Math.round(PlayerBase[inz].time)},
          ${255 - Math.round(PlayerBase[inz].time)},
          1
        )`;
          c.fillRect(0, 0, mx, mh);
          ...
          drawPat(
            ...
            mx,
            mh,
            -mx + Math.round(PlayerBase[inz].clouds),
            0,
            2 * mx,
            300,
          );
        ...
        for (i = 0; i < this.entities.length; i++) {
          ...
          if (this.entities[i].type == "ent" && PlayerBase[inz].timeRatekoff) {
            this.entities[i].action(this.entities[i]);
          }
          // a being promoted to be more than any by simply existing(player render)
          if (
            PlayerBase[inz].Zindex == i ||
            (i == this.entities.length - 2 && this.deolaba == "ycz")
          ) {
            c.drawImage(
              PlayerBase[inz].color,
              PlayerBase[inz].SpX,
              PlayerBase[inz].SpY,
              PlayerBase[inz].w / PlayerBase[inz].size,
              PlayerBase[inz].h / PlayerBase[inz].size,
              PlayerBase[inz].x - PlayerBase[interZept].xc,
              PlayerBase[inz].y - PlayerBase[interZept].yc,
              PlayerBase[inz].w,
              PlayerBase[inz].h,
            );
          }
...
class door {
  constructor(alp, bet, dess, ido, xD, xom) {
    ...
    this.color = "black";
    this.chkCol = (A) => {
      PlayerBase[interZept].room = dess;
      PlayerBase[interZept].x = xD;
      if (PlayerBase[interZept].state == "jumping") {
        PlayerBase[interZept].state = "";
      }
      PlayerBase[interZept].y = xom - PlayerBase[interZept].h;
      PlayerBase[interZept].velY = 0;
      world = eval(localStorage.world);
      world = world[PlayerBase[interZept].room];
      if (xD > mx / 2) {
        if (xD > A + mx / 2) {
          PlayerBase[interZept].xc = A;
        } else {
          PlayerBase[interZept].xc = xD - mx / 2;
        }
      } else {
        PlayerBase[interZept].xc = 0;
      }
    };
  }
}
